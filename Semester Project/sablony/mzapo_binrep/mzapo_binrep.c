...
#define MOTOR_RANGE 40000 //40 kHz
...
int main(int argc, char *argv[])
{
  ...
  /* INICIALIZE THE FINAL VALUE TO THE MIDDLE */
  int final_value = MOTOR_RANGE / 2;
  ...
  while (1)
  {
    ...
    *(volatile uint32_t *)(mem_base + SPILED_REG_LED_RGB2_o) = rgb_knobs_value;
    ...
    /* AKTUALNI POHYB KNOBU */
    int value_change = *(volatile uint32_t *)(mem_base + SPILED_REG_KBDRD_KNOBS_DIRECT_o);
    final_value += value_change;
    ...
    /* Set led line */
    int led = 0xff000000;
    uint32_t led_line = *(volatile uint32_t *)(mem_base + SPILED_REG_KNOBS_8BIT_o);
    for (int i = 0; i < 24 * (led_line / 16777215.); i++)
    {
      led = led >> 1;
    }
    printf("FINAL VALUE %d\r", final_value);
    // printf("HERE %f\n", 24 * (led_line / 16777215.));
    // printf("TEST int %d\n", led_line);
    // printf("LED int %d\n", led);
